...
export class MobileAttendanceService {
  ...
  static async submitAttendance(payload: any) {
    ...
    const todayZeroHour = new Date(dateString);
    ...
    if (!routine || !routine.is_active) {
      throw new Error(
          `System is offline today (${currentDayName}). No tracking required.`);
    }
    ...
    if (!isSessionActive) {
      throw new Error(
          `The ${requestedSession} session is not active for today (${
              currentDayName}).`);
    }

    // 4. CEK BATAS WAKTU (PRAYER CACHE)
    const cache =
        await prisma.tbl_prayers.findUnique({where: {date: todayZeroHour}});

    if (!cache) {
      throw new Error(
          'Missing today\'s schedule cache. Please run sync from the Bridges page.');
    }

    const sessionTimeStr =
        cache[requestedSession.toLowerCase() as keyof typeof cache] as string;
    ...
    if (diffMins < 0) {
      throw new Error(
          `The ${requestedSession} session has not started yet. Starts at ${
              sessionTimeStr}.`);
    }
    ...
    if (diffMins <= 20) {
      ...
      if (method !== 'Manual') {
        throw new Error(
            `QR Scanner is closed. Elapsed +20mins. Proceed with Manual input.`);
      }
      validatedStatus = 'Late';
    } else {
      throw new Error(`The ${
          requestedSession} session is permanently closed (Elapsed +60mins).`);
    }
    ...
    if (existing)
      throw new Error(
          `Student has already checked in for ${requestedSession} today.`);
