...
export default function StudentDetailPage({ params }: { params: Promise<{ slug: string, nanoId: string }> }) {
  ...
  if (!v.currentStudent) return <div className="p-6 text-foreground font-bold text-center mt-20">Siswi Tidak Ditemukan</div>;
  ...
  return (
    <div className="flex flex-col w-full h-full p-6 max-w-350 mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center gap-4 border-b border-white/5 pb-4">
        <Button variant="outline" size="icon" onClick={() => router.back()} className="rounded-full bg-white/5 border-white/10 hover:bg-white/10 text-white shrink-0 transition-transform hover:-translate-x-1">
        ...
        <div className="flex flex-wrap items-center text-xs font-semibold tracking-wide uppercase">
          ...
          <span className="text-muted-foreground cursor-pointer hover:text-white transition-colors" onClick={() => router.back()}>{v.classData?.name}</span>
          <span className="text-muted-foreground mx-2">/</span>
