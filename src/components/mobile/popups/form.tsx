...
const formSchema = z.object({
  status: z.preprocess(
    ...
    z.string().refine((val) => ['Menstruation', 'Pure'].includes(val), {
      message: "Please choose an attendance status first",
    })
  ),
...
export function Form({ isOpen, setIsOpen, dataStudent, setPick, setSuccessPopup, sholat }: FormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: { status: undefined as any, remarks: '' },
  });
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogContent className="w-[92%] max-w-sm rounded-3xl bg-[#151419] border-none text-white p-6 shadow-2xl overflow-y-auto max-h-[90vh] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
