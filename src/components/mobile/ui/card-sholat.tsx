...
export const CardSholat = ({ className = '' }: { className?: string }) => {
  ...
  return (
    <div className={`relative w-full bg-[#1F1E23] border border-white/5 rounded-3xl py-4 px-6 flex items-center justify-between gap-6 transition-all duration-500 shadow-sm ${className}`}>
      ...
      <div className="flex flex-col items-end justify-center flex-1">
        ...
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-tighter">WIB</span>
        </div>
      </div>
    </div>
  );
