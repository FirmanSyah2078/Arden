...
export const Manager = ({ className = '', onOpenHistory, onLogout }: ManagerProps) => {
  ...
  // STATE SMART DOCK nya! (=^･ω･^=)
  const [isDockVisible, setIsDockVisible] = useState(true);
  const handleDockVisibility = (visible: boolean) => setIsDockVisible(visible);

  const qrRef = useRef<QrHandle>(null);
  const { activeScanner } = useSholat();

  // ALARM 1: Bangunin dock setiap kali ganti mode nya! ฅ^•ﻌ•^ฅ
  useEffect(() => {
    setIsDockVisible(true);
  }, [mode]);

  // ALARM 2: Bangunin dock pas popup form ditutup nya!
  useEffect(() => {
    if (!inputFormOpen) {
      setIsDockVisible(true);
    }
  ...
  // FUNGSI BARU: Wrapper untuk setSearch biar dock-nya ikutan bangun nya! (=^･ω･^=)
  const handleSetSearch = (val: string) => {
    setSearch(val);
    if (val === '') {
      setIsDockVisible(true); // Pas klik 'X' (clear), dock harus muncul!
    }
  };
      ...
      <div className="relative w-full h-full flex flex-col transition-all duration-500 ease-in-out">
        {mode === 'scan' ? (
          ...
          <div className="w-full h-full p-5 pt-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col">
            <Manual
              search={search}
              setSearch={handleSetSearch} // Pakai wrapper handleSetSearch nya! ฅ^•ﻌ•^ฅ
              data={data}
              isLoading={isLoading}
              onFocus={() => { }}
              onBlur={() => { }}
              ...
              onScrollDirectionChange={handleDockVisibility}
            />
          </div>
        )}
      </div>

      {/* ACTION LAYER - Posisi dinamis berdasarkan isDockVisible nya! */}
      <div className={`absolute ${isDockVisible ? 'bottom-8' : '-bottom-30'} left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ease-in-out ${isCamActive ? 'opacity-60' : 'opacity-100'}`}>
        <div className="flex items-center bg-white/5 backdrop-blur-2xl border border-white/10 p-1.5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] gap-1.5">
          ...
          <Button
            variant="secondary"
            onClick={mode === 'scan' ? handleCamAction : () => handleSetSearch('')} // Pakai handleSetSearch juga di sini nya!
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="h-10 w-10 rounded-full bg-white/3 hover:bg-white/10 text-white transition-all active:scale-95 group outline-none flex items-center justify-center border border-white/10 shrink-0 shadow-sm">
