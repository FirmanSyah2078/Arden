...
import { 
  AlertTriangle, Trash2, Monitor, ShieldCheck, KeyRound, 
  Loader2, CheckCircle2, CircleDashed, Eye, EyeOff, XCircle, Save
} from "lucide-react"
...
export default function SecurityForm() {
  ...
  const inputClass = "w-full rounded-md border border-white/10 bg-black/20 px-3 py-2.5 pr-10 text-[13px] text-white placeholder:text-white/30 outline-none focus:outline-none focus-visible:ring-1 focus-visible:ring-indigo-500/50 focus-visible:border-indigo-500/50 hover:border-white/30 shadow-none transition-all duration-300 tracking-wide"
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div> 
            ...
            <div className={cn("transition-all duration-300 overflow-hidden flex items-center gap-3", passwords.new.length > 0 ? "h-5 opacity-100 mt-3" : "h-0 opacity-0 mt-0")}>
              <span className={cn("text-[10px] font-medium shrink-0 flex items-center min-w-10", strengthUI.text)}>
          ...
          <div> 
            ...
            <div className="relative group">
              <input
                ...
                className={cn(
                  "w-full rounded-md border bg-black/20 px-3 py-2.5 pr-10 text-[13px] text-white placeholder:text-white/30 focus:outline-none focus:ring-1 transition-all tracking-wide shadow-none",
                  passwords.confirm.length > 0 && !isMatch 
                    ? "border-red-500/50 focus:border-red-500 focus:ring-red-500/50" 
                    : "border-white/10 focus:border-indigo-500/50 focus:ring-indigo-500/50 hover:border-white/30"
                )}
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          >
            {isSubmitting ? (
               ...
               <><Save className="size-4 group-hover:ar-bounce-x" /> Save Password</>
            )}
          </button>
        </div>
      </div>
        ...
        <div className="rounded-lg border border-white/10 bg-black/20 p-5 transition-all duration-300 hover:border-red-900/30 hover:bg-red-950/10">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button className="group flex shrink-0 items-center gap-2 rounded-md border border-red-500/20 bg-red-500/10 px-4 py-2 text-[12px] font-medium text-red-400 transition-all duration-300 hover:bg-red-500/20 hover:border-red-500/40 hover:text-red-300 hover:shadow-[0_0_15px_rgba(239,68,68,0.1)]">
              <Trash2 className="size-3.5 transition-transform duration-300 group-hover:ar-shake" />
              Delete
            </button>
          </div>
        </div>
